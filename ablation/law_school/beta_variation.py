...
# print(np.max(X_train_sc, axis=0), np.min(X_train_sc, axis=0), np.max(X_test_sc, axis=0), np.min(X_test_sc, axis=0)) 
d = X_train_sc.shape[1]

eta_model_oracle = estimate_eta(X_train_sc, y_train)

# Highly Non-Uniform Alpha Mapping based on real-world manipulability:
# cost_map = {
#     'lsat': 5.0,     
#     'ugpa': 1.0,     
#     'zgpa': 10.0       
...
fixed_alpha_np = np.array([cost_map.get(f, 100.0) for f in feature_cols])
...
plt.xscale('log')
plt.xlabel('Beta values (Log Scale)')
...
plt.savefig('lawschool_beta_sim_update.png', dpi=300)
pd.DataFrame({'Scale': beta_values, 'err_f': results_b, 'err_fs': results_s, 'err_fimp': results_i}).to_csv('lawschool_uniform_alpha_sim.csv', index=False)


print("Experiment complete. Results saved to 'law_school_beta_sim_update.png' and 'law_school_beta_sim.csv'.")