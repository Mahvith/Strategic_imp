...
def load_and_process_lawschool(datapath='law_school_clean.csv'):
    ...
    feature_cols = ['lsat', 'ugpa','zfygpa', 'zgpa'] # 'decile1b', 'decile3'
...
#     'ugpa': 5.0,     # Immutable once graduated
#     'zfygpa': 1000.0,   # Immutable past 1st year
#     'zgpa': 10.0,       # Can be somewhat improved before graduation
#     'decile1b': 1000.0,  # Zero-sum class rank, very hard
#     'decile3': 1000.0,   # Zero-sum class rank, very hard
# }
cost_map = {
    'lsat': 1.0,      # Prep courses can help, but hard
    'ugpa': 1.0,     # Immutable once graduated
    'zfygpa': 1.0,   # Immutable past 1st year
    'zgpa': 1.0,       # Can be somewhat improved before graduation
    # 'decile1b': 1.0,  # Zero-sum class rank, very hard
    # 'decile3': 1.0,   # Zero-sum class rank, very hard
}
base_alpha = np.array([cost_map.get(f, 100.0) for f in feature_cols])

alpha_multipliers = [1.0, 100.0, 500, 1000, 2500, 5000, 7000, 10000.0]
...
plt.savefig('lawschool_uniform_alpha_sim.png', dpi=300)
