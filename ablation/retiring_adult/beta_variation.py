...
def build_dataloaders(X_scaled, Y, batch_size=512):
    ...
    train_dl = DataLoader(TensorDataset(X_train_t, Y_train_t), batch_size=batch_size, shuffle=True) ## not working
    return train_dl
...
X_train, X_test, y_train, y_test = load_and_process_acs_income(datapath="retiring_adult_data.csv")
...
# beta_values = [0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0]
...
plt.title('Effect of Varying Beta (Allowed Manipulation Cost) - Retiring Adult Dataset')
plt.legend()
...
plt.savefig('retiring_adult_beta_sim.png', dpi=300)
pd.DataFrame({
    'Beta': beta_values,
    'err_f': results_b,
    'err_fs': results_s,
    'err_fimp': results_i
}).to_csv('retiring_adult_beta_sim.csv', index=False)

print("Experiment complete. Results saved to 'retiring_adult_beta_sim.png' and 'retiring_adult_beta_sim.csv'.")