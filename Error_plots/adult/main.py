...
def load_and_process_adult(datapath='adult.csv'):
    ...
    # ed_col = 'educational-num' if 'educational-num' in df.columns else 'education-num'
...
if __name__ == "__main__":
    ...
    train_dl, val_dl = build_dataloaders(X_train_sc, y_train)
    d = X_train_sc.shape[1]
    
    print("\nTraining Eta (True Probability) Model...")
    eta_model = estimate_eta(X_train_sc, y_train, X_test_sc, y_test)
    ...
    # Define test grids for beta and alpha
    ...
    for beta in betas_to_test:
        ...
        for name, alpha in alpha_configs.items():
            print(f"\n--- Configuration: {name} (Alpha: {alpha.cpu().numpy().round(1)}) ---")
            
            # A. Evaluate Bayes Optimal (f*) under current (alpha, beta) conditions
            err_bayes = compute_err_imp(model_bayes, X_test_sc, eta_model, alpha.cpu().numpy(), beta)
            ...
            err_strat = compute_err_imp(model_strat, X_test_sc, eta_model, alpha.cpu().numpy(), beta)
            ...
            err_imp = compute_err_imp(model_imp, X_test_sc, eta_model, alpha.cpu().numpy(), beta)
