...
def load_and_process_adult(datapath='adult.csv'):
    ...
    # # General one-liner using df.columns.get_loc
    # imp_indexes = [df.columns.get_loc(f) for f in features]
    # print(f"Selected features and their column indices: {dict(zip(features, imp_indexes))}")
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test, improvable_indexes = load_and_process_adult('adult.csv')
    ...
    plt.tight_layout()
    plt.savefig('sample_complexity_curves_with_variance_with_pacimp.png', dpi=300)
    plt.show()
