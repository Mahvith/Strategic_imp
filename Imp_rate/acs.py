...
X, y = load_and_process_acs_income('retiring_adult_data.csv')

X_train, X_test, y_train, y_test = data_split(X, y, 0.3)
train_dl = build_dataloaders(X_train, y_train, 1024)
...
beta = 1.0
alpha = torch.tensor([0.7, 0.1, 1.0], dtype=torch.float32, device=device)
...
for name, model in models.items():
    ...
    log_experiment(name ,w,b,alpha_np,beta,n_manip,n_imp,rate, "acs_Imp_rate.csv")