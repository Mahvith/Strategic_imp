...
X_train, X_test, y_train, y_test = load_and_process_synthetic_data("synthetic_dataset_100.csv")

## Not performing another scaling! 
...
fixed_alpha_np = np.array([1, 1, 1, 1, 1, 1, 1, 1])
...
# beta_values = [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0]

beta_values = [0.01, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0]
...
print("Starting Beta Variation Experiment...")
for current_beta in tqdm(beta_values, desc="Beta Values"):

    # Bayes
    model_linear = LinearClassifier(d, 1).to(device)
    opt_l = torch.optim.Adam(model_linear.parameters(), lr=0.01)
    optimal_linear_classifier(model_linear, train_dl, opt_l, C=1.0).train(n_epochs=50)
    ...
    ImprovementAwareTrainer(model_imp, train_dl, opt_imp, eta_model_oracle, fixed_alpha_t, current_beta, C=1.0).train(n_epochs=50)

    # Compute Improvement Error for the current Beta
    err_l = compute_err_imp(model_linear, X_test_sc, eta_model_oracle, fixed_alpha_np, current_beta)
    ...
    print(f"\nBeta: {current_beta:5.2f} | f*: {err_l:.4f} | f*_s: {err_s:.4f} | f*_imp: {err_i:.4f} | Trend Holds: {trend_holds}")

# ==========================================
# PLOTTING AND SAVING
# ==========================================
plt.figure(figsize=(10, 6))
plt.plot(beta_values, results_l, marker='o', label=r'Optimal Linear ($f^*$)')
...
# If you have beta=0.0, log scale will throw a warning, so we use 'symlog' or linear.
# Symlog handles 0.0 gracefully while scaling the rest like a log scale.
plt.xscale('symlog', linthresh=0.1)
plt.xlabel('Beta Value (Log Scale)')
plt.ylabel('Improvement Error (err_imp)')
plt.title('Effect of Varying Beta (Allowed Manipulation Cost) - Synthetic Dataset')
plt.legend()
...
plt.savefig('synthetic_beta_sim.png', dpi=300)
pd.DataFrame({
    'Beta': beta_values,
    'err_f': results_l,
    'err_fs': results_s,
    'err_fimp': results_i
}).to_csv('synthetic_beta_sim.csv', index=False)
    
print("Experiment complete. Results saved to 'synthetic_beta_sim.png' and 'synthetic_beta_sim.csv'.")